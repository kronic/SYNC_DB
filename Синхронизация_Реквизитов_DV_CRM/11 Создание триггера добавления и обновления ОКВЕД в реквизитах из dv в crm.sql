...
/*********************************************/
IF object_id(N'DV_CRM_Upd_Req_OKONH') IS NOT NULL
DROP TRIGGER DV_CRM_Upd_Req_OKONH
GO
/*********************************************/

--Добавляем триггер. добовления  контактов в DV при добовление в CRM.
/*********************************************/
CREATE TRIGGER DV_CRM_Upd_Req_OKONH
...
IF(UPDATE(OKONH))
BEGIN
	DECLARE		@_BEF_OKONH						nvarchar(128)
	DECLARE		@_AFT_OKONH						nvarchar(128)

	-- Получим значение до обноления
	/*********************************************/		
	SELECT	@_BEF_OKONH					=		UPD.OKONH
	FROM	DELETED AS UPD
	/*********************************************/	
	...
	/*********************************************/
	SELECT 
	TOP 1		@_AFT_OKONH				=		UPD.OKONH
	FROM		INSERTED as UPD;
	/*********************************************/

	IF(	@_BEF_OKONH	=	@_AFT_OKONH 
	OR  @_AFT_OKONH	IS	NULL 
		) RETURN
