...
/*********************************************/
IF object_id('CRM_DV_RS', 'TR') IS NOT NULL
DROP TRIGGER CRM_DV_RS
GO
/*********************************************/

--Добавляем триггер. добовления  контактов в DV при добовление в CRM.
/*********************************************/
CREATE TRIGGER CRM_DV_RS
...
BEGIN
	...
	DECLARE		@_RS						varchar(20) -- DV type nvarchar(128) привод типа и длинны не нужен
	...
	DISABLE TRIGGER [DV_CRM_Upd_Req_RS]
	/*********************************************/	
	...
	/*********************************************/		
	ALTER TABLE [Copy_DV].[dbo].[dvtable_{c78abded-db1c-4217-ae0d-51a400546923}]
	ENABLE TRIGGER [DV_CRM_Upd_Req_RS]
	/*********************************************/	
